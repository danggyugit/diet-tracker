...
# ─── 감량 강도 옵션 ──────────────────────────────────────────
DEFICIT_OPTIONS = {
    "🟢 가벼운 감량 (-500) — 주 0.45kg": 500,
    "🟡 보통 감량 (-700) — 주 0.64kg": 700,
    "🔴 강한 감량 (-1000) — 주 0.9kg": 1000,
    "⚪ 체중 유지": 0,
}
...
with st.form("profile_form"):
    ...
    with col2:
        ...
        activity_level = st.selectbox(
            "활동 수준", list(ACTIVITY_MULTIPLIERS.keys()),
    ...
    st.markdown("---")
    st.markdown("**감량 강도**")
    deficit_choice = st.radio("감량 강도 선택", DEFICIT_KEYS, index=default_deficit_idx, label_visibility="collapsed")
...
n1.metric("🍚 탄수화물", f"{carbs_g}g")
n2.metric("🥩 단백질", f"{protein_g}g", delta=f"체중x{protein_mult}g")
n3.metric("🧈 지방", f"{fat_g}g", delta="30%")

st.caption(
    f"단백질: {weight:.0f}kg x {protein_mult}g = {protein_g}g ({protein_cal}kcal) · "
    f"지방: {daily_target:,} x 30% / 9 = {fat_g}g ({fat_cal}kcal) · "
